...
@wp.kernel
def update_pml_h(state: EMState, props: Properties):
    i, j, k = wp.tid()
    nx = state.Ex.shape[0]
    ny = state.Ex.shape[1]
    nz = state.Ex.shape[2]
    ...
    state.Hx[i, j, k] = props.CHA_X[i, j, k] * state.Hx[i, j, k] - props.CHB_X[
        i, j, k
    ] * ((dEz_dy * props.INV_K_Y[j] + psi_y) - (dEy_dz * props.INV_K_Z[k] + psi_z))
    ...
    state.Hy[i, j, k] = props.CHA_Y[i, j, k] * state.Hy[i, j, k] - props.CHB_Y[
        i, j, k
    ] * ((dEx_dz * props.INV_K_Z[k] + psi_z) - (dEz_dx * props.INV_K_X[i] + psi_x))
    ...
    state.Hz[i, j, k] = props.CHA_Z[i, j, k] * state.Hz[i, j, k] - props.CHB_Z[
        i, j, k
    ] * ((dEy_dx * props.INV_K_X[i] + psi_x) - (dEx_dy * props.INV_K_Y[j] + psi_y))



@wp.kernel
def update_pml_e(state: EMState, props: Properties):
    i, j, k = wp.tid()
    nx = state.Ex.shape[0]
    ny = state.Ex.shape[1]
    nz = state.Ex.shape[2]
    ...
    state.Ex[i, j, k] = props.CEA_X[i, j, k] * state.Ex[i, j, k] + props.CEB_X[
        i, j, k
    ] * ((dHz_dy * props.INV_K_Y[j] + psi_y) - (dHy_dz * props.INV_K_Z[k] + psi_z))
    ...
    state.Ey[i, j, k] = props.CEA_Y[i, j, k] * state.Ey[i, j, k] + props.CEB_Y[
        i, j, k
    ] * ((dHx_dz * props.INV_K_Z[k] + psi_z) - (dHz_dx * props.INV_K_X[i] + psi_x))
    ...
    state.Ez[i, j, k] = props.CEA_Z[i, j, k] * state.Ez[i, j, k] + props.CEB_Z[
        i, j, k
    ] * ((dHy_dx * props.INV_K_X[i] + psi_x) - (dHx_dy * props.INV_K_Y[j] + psi_y))
