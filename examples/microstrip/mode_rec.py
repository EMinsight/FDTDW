...
DDZ2 = NZ - 20
sim.init_detector(
    shape=(NUD, NVD), OFFSETX=DDX, OFFSETY=DDY, OFFSETZ=DDZ1, plane="xy", full=False
)
sim.init_detector(
    shape=(NUD, NVD), OFFSETX=DDX, OFFSETY=DDY, OFFSETZ=DDZ2, plane="xy", full=False
)
sub_height = 10
eps_r_air = 1
eps_r_sub = 4.4
...
dft_detector_1 = sim.get_dft_detector(target_freqs, idx=0, save=False)
dft_detector_2 = sim.get_dft_detector(target_freqs, idx=1, save=True)
...
for dft_detector in (dft_detector_1, dft_detector_2):
    ...
    for field in fields:
        ...
        plt.tight_layout()
        filename = f"profile_{field}_{ND}.png"
